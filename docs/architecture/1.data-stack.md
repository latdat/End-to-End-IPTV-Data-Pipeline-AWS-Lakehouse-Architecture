...
1. **Schema normalization** — Lựa chọn và ép kiểu (cast) các trường thông số từ JSON nguyên gốc. Bổ sung ngày tháng phát sinh sự kiện (`event_date`).
2. **Validate and Flag invalid records** — Áp dụng kiểm thử các bản ghi bất thường (như `duration <= 0`, thiếu `contract_id` hoặc MAC, `App = BHD`), thay vì loại bỏ thì thực hiện lưu dấu nguyên nhân lỗi vào mảng thông tin `fraud_reasons`.
3. **Normalize MAC address** — Quy trình chuyển đổi dữ liệu địa chỉ MAC nguyên bản sang form chuẩn `XX:XX:XX:XX:XX:XX` ký tự in hoa.
4. **Deduplication** — Lược bỏ các dòng trùng lặp hoàn toàn dựa theo định danh khóa `event_id` (vẫn áp dụng drop duplicate do hệ quả trực tiếp từ retry).
5. **Fraud detection** — Cảnh báo bắt lỗi theo ngưỡng `FRAUD_MAC_THRESHOLD`: định danh các tài khoản (`contract_id`) chia sẻ hợp đồng bằng việc ghi nhận lượng thiết bị truy cập bất thường. Cập nhật vào thuộc tính `is_fraudulent` (khiến cho bản ghi đánh dấu cảnh báo). Toàn bộ dữ liệu (cả valid lẫn fraud) đều được lưu trữ 100% về Silver.
...
- **Staging layer (lv1_staging)**: Dữ liệu nguồn `stg_iptv__logs_all` được trích xuất từ kho chứa raw, sau đó chia làm 2 nhánh tách biệt dựa trên chỉ báo `is_fraudulent`:
    - `stg_iptv__logs`: Bảng staging lưu giữ bản ghi sạch (valid) phục vụ tính toán chỉ số cho các Data Mart cấp trên.
    - `stg_iptv__logs_fraud`: Bảng staging tập trung ghi nhận các trường hợp mắc lỗi / gian lận, dùng cho báo cáo Audit & Security.
- **Mart layer (lv3_mart)**: Ví dụ như (`fct_daily_views`), tập trung xử lý nhánh dữ liệu sạch `stg_iptv__logs` để gom nhóm (aggregate) tạo các thông số như `total_sessions`, `unique_contracts`, `total_hours`, tạo thành bảng báo cáo lưu trữ định dạng `table`.
...
```text
  ...
  │                                  • tag missing fields & test traffic        │
  ...
  │                              stg_iptv__logs_all (view)                      │
  │                              ├──► stg_iptv__logs (clean log)                │
  │                              │      └──► fct_daily_views (table)            │
  │                              └──► stg_iptv__logs_fraud (fraud tracking)     │
  └─────────────────────────────────────────────────────────────────────────────┘
```
