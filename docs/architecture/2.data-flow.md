...
| *- Không có -* | `is_fraud_flag` | *(Được lọc bỏ)* | Cột phái sinh tạo bởi thuật toán Window Function trên Glue để đánh dấu dữ liệu rác/spam. |
...
flowchart TD
    ...
    B{Drop Nulls}
    B -->|Bản ghi có CONTRACT hoặc MAC bị Null| X1([Discard])
    B -->|Hợp lệ| D
...
| Drop Nulls | 48,457,493 | 6 | `CONTRACT IS NULL` hoặc `MAC IS NULL` |
| Filter Contract Format | 45,976,306 | 2,481,187 | Contract ID không đúng định dạng quy chuẩn |
| Filter Duration | 45,903,896 | 72,410 | Thời lượng xem `<= 0` hoặc `> 86,400` giây (24h) |
| Filter Test App | 45,903,895 | 1 | Dữ liệu phát sinh từ nội bộ (App = 'BHD') |
| Parse MAC Address | 45,903,867 | 28 | Lỗi cấu trúc, không thể giải mã MAC address |
