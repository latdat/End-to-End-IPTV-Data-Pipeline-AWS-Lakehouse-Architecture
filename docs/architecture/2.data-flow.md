...
```mermaid
flowchart TD
    A["Nguồn (S3 Bronze)"] --> B{Contract<br>Null?}
    B -->|Có| X1([Discard])
    B -->|Không| D{Contract<br>lỗi format?}
    D -->|Có| X2([Discard])
    D -->|Không| F{Duration<br><=0 hoặc >24h?}
    F -->|Có| X3([Discard])
    F -->|Không| H{App =<br>'BHD'?}
    H -->|Có| X4([Discard])
    H -->|Không| I["Chuẩn hóa<br>MAC Address"]
    I --> J{Lỗi parse<br>MAC?}
    J -->|Có| X5([Discard])
    J -->|Không| K["Khử trùng lặp<br>(event_id)"]
    K --> L["Đánh dấu Fraud<br>(is_fraud_flag)"]
    L --> M(["Đích (S3 Silver)"])

    classDef discard fill:#ffe6e6,stroke:#cc0000,stroke-width:1px,color:#990000,font-weight:bold
    class X1,X2,X3,X4,X5 discard
